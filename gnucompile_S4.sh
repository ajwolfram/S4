#!/bin/bash
# cb@caltech.edu Aug 2018
# This might not be the easiest way to compile a system specific MEEP installation isolated in
# an anaconda environment. I imagine that some parts are incorrect or redundant, but
# all tests and checks pass as of Aug 1, 2018.

# This uses the mkl libraries from intel, which are default with the anaconda packages -- one might get better performance using intel's conda channel
# This would be done by using '-c intel' when installing and the descending precedence of channels
...
export PATH="${CONDA_PREFIX}/bin:${HOME}/bin:/bin:/usr/bin"
...
export LIBRARY_PATH="$SRCBUILDDIR/S4/S4/lib/:${MKLROOT}:$SRCBUILDDIR/SuiteSparse/lib/:${CONDA_PREFIX}/lib/:${CONDA_PREFIX}/lib/openmpi/"
export CPPFLAGS="-I${MKLROOT}/include/fftw/ -I${CONDA_PREFIX}/include/  -I${SRCBUILDDIR}/S4/S4/include/ -I${SRCBUILDDIR}/SuiteSparse/include/"
export CPATH="$CPATH:{MKLROOT}/include/fftw/:${CONDA_PREFIX}/include:${CONDA_PREFIX}/include/gsl/:${CONDA_PREFIX}/lib/python3.6/site-packages/mpi4py/include:${CONDA_PREFIX}/lib/python3.6/site-packages/numpy/core/include/numpy/:${SRCBUILDDIR}/S4/S4/include/:${SRCBUILDDIR}/SuiteSparse/include/"
